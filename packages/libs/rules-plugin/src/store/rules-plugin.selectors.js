...
export function createRulesPluginSelectors(rulesPluginStoreSliceIds) {
  ...
  const getIsMapPartiallyOverUnavailableJurisdiction = state =>
    storeSelector(state).isMapPartiallyOverUnavailableJurisdiction
  ...
  const getUnclassifiedLayersStyles = createSelector(
    getClassifiedLayersStyles,
    layers => layers.filter(layer => layer.id.includes(NON_CLASSIFIED))
  )

  const getRulesets = createSelector(
    getMapJurisdictions,
    jurisdictions => {
      const rulesets = []
  ...
  const getRulesetsIds = createSelector(
    getRulesets,
    rulesets => rulesets.map(ruleset => ruleset.id).join()
  )

  const getSelectedRulesetsBySource = createSelector(
    getSelectedRulesets,
    selectedRulesets => {
  ...
  const getVisibleRulesetsIds = createSelector(
    getRulesets,
    getSelectedRulesetsBySource,
    (rulesets, selectedRulesetsBySource) => {
      return rulesets
  ...
  const getParsedRulesetInformation = createSelector(
  ...
  const getMapBoundsGeometryCenter = createSelector(
