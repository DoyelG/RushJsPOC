...
import {
  ...
  isolatedIcon,
  isolatedMenu,
  jurisdictionWrapperSmall
} from './RulesetsMenu.styles.scss'
...
export const RulesetsMenu = ({
  ...
  useEffect(() => {
    ...
    const hasBeenHighlightedRulesetsChanged =
      Object.keys(highlightedRulesets).join() !== Object.keys(selectedRulesets.highlighted).join()
  ...
  const collapsibleMenu = () => (
    <Collapse
      in={isMenuOpen}
      className={`${collapse} ${isolated ? isolatedMenu : ''}`}
      transition="auto"
      unmountOnExit
      style={rulesetsMenuPortalModeEnabled ? rulesetsMenuPortalInlineStyles : {}}
    >
      <div className={collapseWrapper}>
        <div className={rulesetsMenuHeader}>{airmapIntl.translateMessage(RulesetsMenuMessages.rules_header)}</div>
        <div className={jurisdictionsCardContent}>
          {isMapPartiallyOverUnavailableJurisdiction && <WarningInfoBox />}
          <div className={`${jurisdictionWrapper} ${useSmallStyles ? jurisdictionWrapperSmall : ''}`}>
            {isViewingRulesetRulesInfo ? (
              <RulesetsInfoPanel
                handleHideRulesetInfo={() => setIsViewingRulesetRulesInfo(false)}
                isFetchingRulesetInformation={isFetchingRulesetInformation}
                rulesetInformation={rulesetInformation}
              />
            ) : (
              Object.keys(jurisdictions).map(jurisdictionKey => {
                const jurisdiction = jurisdictions[jurisdictionKey]
                return (
                  <div className={jurisdictionRulesets} key={jurisdictionKey}>
                    <h3 className={jurisdictionTitle}>{jurisdiction.name}</h3>
                    <RulesetsList
                      handleViewRulesInfo={handleViewRulesInfo}
                      handleSelectRuleset={handleSelectRuleset}
  ...
  return showRulesetsMenu && Object.keys(jurisdictions).length > 0 ? (
    <div className={rulesetsMenuWrapper}>
      <div className="toggleable-menu" id={menuButtonId} onClick={() => setIsMenuOpen(!isMenuOpen)}>
        <IconButton
          className={`${rulesIcon} ${isolated ? isolatedIcon : ''}`}
          onClick={() => {}}
          id="rulesets-menu-icon"
          icon={
            <SvgIcon viewBox={'0 0 20 24'}>
              <Layer width={21} height={24} fillColor={'#848f98'} />
            </SvgIcon>
          }
        />
      </div>
      {rulesetsMenuPortalModeEnabled ? (
        <Portal id="rulesets-plugin-collapsible-menu">{collapsibleMenu()}</Portal>
      ) : (
        collapsibleMenu()
      )}
    </div>
