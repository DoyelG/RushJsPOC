...
import {
  ...
  appliedFilterChip,
  chipActive,
  advisoriesFilterWrapper,
  title,
  showInactiveAdvisoriesWrapper,
  showInactiveAdvisoriesText,
  switchChecked,
  disclaimerWrapper,
  disclaimerTitle,
  disclaimerBody,
  ...
  footerRight,
  divider
} from './AdvisoriesFilter.styles.scss'
...
import {
  defaultTimeRangeFilterValues,
  predefinedRanges
} from 'modules/shared-components/TimeRangeFilter/TimeRangeFilter.constants'
...
export const AdvisoriesFilter = ({
  ...
  useEffect(
    function initTheAppliedAdvisoriesFilter() {
  ...
  useEffect(
    function setPredefinedRangeBasedOnDate() {
      if (!isTodaySelected) {
  ...
  useEffect(
    function validateTimeRangeValues() {
      const now = moment()
  ...
  useEffect(
    function setSwitchValueBasedOnStore() {
  ...
  return (
    <Popover
      open={openPopover}
      handleOnClose={() => setOpenPopover(false)}
      classNameBody={popoverBody}
      openElement={
        <div
          ...
          id={advisoriesFilterButtonId}
        >
          <IconButton
            id="filter-advisories-menu-icon"
            className={openButton}
            onClick={() => {}}
            icon={
              <>
                <div className={`${appliedFilterChip} ${openPopover ? chipActive : ''}`}>
                  <Clock />
                  <span>{appliedAdvisoriesFilter.formattedValue}</span>
                </div>
                <Filter color={openPopover ? theme.palette.primary.main : theme.rulesPlugin.neutral3_opacity_05} />
              </>
            }
          />
        </div>
      ...
      content={
        <div className={advisoriesFilterWrapper} id={`sel-wrapper-advisories-filter`}>
          <h2 className={title}>{airmapIntl.translateMessage(AdvisoriesFilterMessages.filterAdvisoriesTitle)}</h2>

          <DatePicker id="advisories-filter" value={dateFilterValue} handleOnChange={setDateFilterValue} />

          <TimeRangeFilter
            id="advisories-filter"
            value={timeFilterValue}
            handleChangeCustomRange={handleChangeCustomRange}
            handleChangePredefinedRange={setSelectedPredefinedRange}
          ...
          <div className={showInactiveAdvisoriesWrapper}>
            <label className={showInactiveAdvisoriesText} htmlFor="toggle-inactive-advisories">
              {airmapIntl.translateMessage(AdvisoriesFilterMessages.showInactiveAdvisories)}
            </label>

            <Switch
              id="toggle-inactive-advisories"
              checked={showInactiveAdvisoriesChecked}
              onChange={toggleShowInactiveAdvisories}
              color="primary"
              classes={{ iconChecked: switchChecked }}
            />
          </div>
          ...
          <div className={disclaimerWrapper}>
            <h3 className={disclaimerTitle}>{airmapIntl.translateMessage(AdvisoriesFilterMessages.disclaimer)}</h3>
            <p className={disclaimerBody}>{airmapIntl.translateMessage(AdvisoriesFilterMessages.disclaimerBody)}</p>
          </div>
        </div>
      }
      footer={
        <div className={footer}>
          <RectangleButton
            id={`sel-button-advisories-filter-reset`}
            onClick={handleResetValues}
            className={button}
            variant="secondary"
            size="small"
            buttonText={airmapIntl.translateMessage(AdvisoriesFilterMessages.reset)}
          />

          <div className={footerRight}>
            <RectangleButton
              id={`sel-button-advisories-filter-close`}
              onClick={() => setOpenPopover(false)}
              className={button}
              variant="secondary"
              size="small"
              buttonText={airmapIntl.translateMessage(AdvisoriesFilterMessages.close)}
            />

            <RectangleButton
              id={`sel-button-advisories-filter-apply`}
              variant="primary"
              size="small"
              onClick={handleApplyValues}
              className={button}
              buttonText={airmapIntl.translateMessage(AdvisoriesFilterMessages.apply)}
            />
          </div>
        </div>
      }
