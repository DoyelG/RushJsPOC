...
import { apiKey, rulesetSourceUrl, rulesetClassifications, sourceTypes, layerTypes, layerColors, measurementUnits, geometryTypes, layerColorsOrder, rulesetSelectionTypes, layersIds, layerFilters, rulesetsStorageKey, temperatureUnits, appliedAdvisoriesFilterStorageKey, shouldShowInactiveAdvisoriesStorageKey, selectableRulesets, controlledAirspaceSubTypes } from '../constants/rules-plugin.constants';
import { MeasurementTypes } from 'modules/core/constants/measurement-types.constants';
const { NON_CLASSIFIED, NON_GEO, HELIPORT, NOTICES_TO_AIRMEN, TEMPORARY_FLIGHT_RESTRICTIONS, BUILDINGS_3D, SPECIAL_USE_AIRSPACE, CONTROLLED_AIRSPACE } = rulesetClassifications;
        ...
        for (let type of layers) {
            ...
            airspaceMapFilter = {
                ...
                [type]: !isControlledAirspaceType
                    ? null
                    : {
                        [CLASSIFICATION_TYPES]: controlledAirspaceSubTypes.reduce((classificationTypes, subType) => ({
                            ...classificationTypes,
                            [subType]: null
                        }), {})
                    }
...
export const getRulesetSource = ({ ruleset, measurementUnits, tileServerAccessToken, appendQuery = '', appendFilter = null }) => {
    const unit = measurementUnits === MeasurementTypes.METRIC ? SI : AIRMAP_UNIT;
    const filter = (ruleset.layers && getAirspaceMapFilter(ruleset.layers)) ||
        appendFilter ||
...
export const setAppliedFilterToLayers = ({ map, timeRange: { start: startTime, end: endTime }, showInactiveAdvisories }) => {
...
const addLayer = ({ rulesetId, classification, layer: { beforeId, ...layer }, map, filterTimeRange = {}, showInactiveAdvisories, ruleset, injectLayerMetadata }) => {
...
const addRuleset = ({ ruleset, map, measurementUnits, classifiedLayersStyles, unclassifiedLayersStyles, filterTimeRange = {}, showInactiveAdvisories = true, tileServerAccessToken, injectLayerMetadata }) => {
    ...
    if (rulesetsLayers.length && !rulesetSourceExists) {
        map.addSource(rulesetId, getRulesetSource({
            ruleset,
            measurementUnits,
...
export const moveToSelectedLayer = ({ map, advisoryId, longitude, latitude }) => {
    const features = map
        ...
        .filter(feature => feature.layer.id.indexOf(AIRMAP) > -1 &&
...
export const getJurisdictionsFromMap = (map, availableDynamicJurisdictions, availableJurisdictionUUIDs) => {
    ...
    const jurisdictions = features.reduce(function getValidParsedJurisdiction(parsedJurisdictions, feature) {
        try {
...
export const groupRulesets = rulesets => {
    const groupedRulesets = rulesets.reduce((groupedRulesets, ruleset) => {
...
export const getDefaultSelectedRulesets = (jurisdictions, selectedRulesets) => {
    ...
    for (const jurisdiction in jurisdictions) {
        ...
        const pickOneRulesetSelected = pickOneRulesets.find(({ id }) => selectedRulesets[PICK_ONE][id] && selectedRulesets[PICK_ONE][id].visibility === 'visible');
