...
import styles, { rulesPluginContainer, rulesPluginHeader, advisoriesRulesetsMenuWrapper } from './RulesPlugin.styles.scss';
import { updateRulesetLayersMap, getJurisdictionsFromMap, moveToSelectedLayer, getMapBoundsBbox, classifyLayers, updateSourceTileURL, createRestrictedZoneDelimiters, setAppliedFilterToLayers, removeRuleset } from './helpers/rules-plugin.helpers';
import { getGeometryCenter } from './helpers/mapboxgl.helpers';
import { jurisdictionLayer, layerTypes, zoomLevels, jurisdictionBaseTileSourceURL } from './constants/rules-plugin.constants';
...
export const RulesPlugin = ({ map, measurementUnits, setClassifiedLayers, classifiedLayersStyles, unclassifiedLayersStyles, mapJurisdictions, setMapJurisdictions, selectedAdvisory, setSelectedAdvisory, clearSelectedAdvisory, fetchClickedAdvisories, clickedAdvisories, isFetchingClickedAdvisories, clearClickedAdvisories, fetchAdvisories, mapBoundsGeometry, setMapBoundsGeometry, selectedRulesetsBySource, visibleRulesetsIds, setIsMapPartiallyOverUnavailableJurisdiction, appliedAdvisoriesFilter, shouldShowInactiveAdvisories, fetchLocationName, fetchWeather, setZoomLevel, setHighlightedLayer, highlightedLayer, clearHighlightedLayer, showLocationInformation, showAdvisoriesFilter, showAdvisoriesList, showRulesetsMenu, showClickedAdvisoriesBox, showInactiveAdvisories, mapBoundsGeometryCenter, locationName, locationWeather, advisoriesData, advisoriesCounter, zoomLevel, jurisdictions, selectedRulesets, setSelectedRulesets, isMapPartiallyOverUnavailableJurisdiction, fetchRulesetInformation, rulesetInformation, isFetchingRulesetInformation, setAppliedAdvisoriesFilter, hideInactiveAdvisories, injectedSelectedRulesets, menusSettings, selectedAdvisorySettings, injectedRulesets, allowDynamicRulesets, tileServerAccessToken, shouldIncludeAccessTokenInTileService, onRulesetsUpdate, injectLayerMetadata, availableDynamicJurisdictionsUUIDs, whiteLabelAvailableJurisdictionUUIDs, allowFetchAdvisoriesData, rulesetsMenuPortalModeEnabled, rulesetsMenuPortalInlineStyles, selectedAdvisoryHasSmallSize, showToggleInjectedRulesetsVisibility, useSmallRulesetsMenuStyles }) => {
    ...
    const handleMapUpdates = useCallback(() => {
        const { availableJurisdictions: incomingMapJurisdictions, unavailableJurisdictions } = getJurisdictionsFromMap(map, availableDynamicJurisdictionsUUIDs, whiteLabelAvailableJurisdictionUUIDs);
        ...
        const hasIncomingMapJurisdictions = incomingMapJurisdictions && Boolean(Object.keys(incomingMapJurisdictions).length);
    ...
    const handleSelectAdvisory = useCallback(advisory => {
    ...
    const handleMapClicks = useCallback(({ target: map, point, lngLat }) => {
        if (selectedAdvisory) {
            hideHighlightedLayer();
        }
        const features = map.queryRenderedFeatures(point);
        const rulesetsIds = visibleRulesetsIds;
        const clickedAdvisory = features.find(feature => feature.layer.id.includes(AIRMAP) && feature.properties.id && feature.properties.category !== 'notification');
        if (clickedAdvisory) {
            const { properties: { category: classification, ruleset_id, id: advisoryId } } = clickedAdvisory;
    ...
    const updateMapLayersOnSourceDataLoaded = useCallback(data => {
    ...
    const applyFilterToLayers = useCallback(({ timeRange = appliedAdvisoriesFilter, showInactiveAdvisories = false }) => {
    ...
    const getAdvisoriesFromVisibleRulesets = useCallback(debounce(() => {
        const zoomLevel = map.getZoom();
        if (visibleRulesetsIds &&
    ...
    }, 1500), [fetchAdvisories, mapBoundsGeometry, visibleRulesetsIds, map, showAdvisoriesFilter, showAdvisoriesList]);
    ...
    useEffect(function setUpJurisdictionBackgroundLayer() {
        if (allowDynamicRulesets) {
            ...
            const hasJurisdictionTileSourceURL = mapJurisdictionSource &&
    ...
    useEffect(function addMapListeners() {
        if (allowDynamicRulesets) {
    ...
    useEffect(function setInitialMapBounds() {
        handleMapBoundsUpdates();
    }, [handleMapBoundsUpdates]);
    const setLocation = useCallback(debounce(() => {
    ...
    }, 1500), [mapBoundsGeometry, map, fetchLocationName, fetchWeather, showLocationInformation]);
    useMemoiseCalls(setLocation, { geometry: mapBoundsGeometry });
    useEffect(function updateInjectedRulesets() {
    ...
    return (_jsxs("div", { className: rulesPluginContainer, children: [_jsxs("div", { className: `${rulesPluginHeader} ${styles[position]}`, children: [showAdvisoriesFilter && (_jsx("div", { children: _jsx(AdvisoriesFilter, { appliedAdvisoriesFilter: appliedAdvisoriesFilter, shouldShowInactiveAdvisories: shouldShowInactiveAdvisories, applyFilterToLayers: applyFilterToLayers, getAdvisoriesFromVisibleRulesets: getAdvisoriesFromVisibleRulesets, showInactiveAdvisories: showInactiveAdvisories, setAppliedAdvisoriesFilter: setAppliedAdvisoriesFilter, hideInactiveAdvisories: hideInactiveAdvisories }, void 0) }, void 0)), _jsxs("div", { className: advisoriesRulesetsMenuWrapper, children: [allowDynamicRulesets && !areInjectedRulesetsHidden && (_jsx("div", { children: _jsx(RulesetsMenu, { jurisdictions: jurisdictions, selectedRulesets: selectedRulesets, setSelectedRulesets: setSelectedRulesets, isMapPartiallyOverUnavailableJurisdiction: isMapPartiallyOverUnavailableJurisdiction, fetchRulesetInformation: fetchRulesetInformation, rulesetInformation: rulesetInformation, isFetchingRulesetInformation: isFetchingRulesetInformation, showRulesetsMenu: showRulesetsMenu, injectedSelectedRulesets: injectedSelectedRulesets, injectedHighlightRulesets: injectedRulesets, isolated: isolated, handleMapUpdates: handleMapUpdates, rulesetsMenuPortalModeEnabled: rulesetsMenuPortalModeEnabled, rulesetsMenuPortalInlineStyles: rulesetsMenuPortalInlineStyles, map: map, useSmallStyles: useSmallRulesetsMenuStyles }, void 0) }, void 0)), showAdvisoriesList && (_jsx("div", { children: _jsx(AdvisoriesHeader, { handleSelectAdvisory: handleSelectAdvisory, advisoriesData: advisoriesData, advisoriesCounter: advisoriesCounter, zoomLevel: zoomLevel, selectedAdvisory: selectedAdvisory, clearSelectedAdvisory: clearSelectedAdvisory }, void 0) }, void 0))] }, void 0)] }, void 0), showToggleInjectedRulesetsVisibility && Boolean(injectedRulesets.length) && (_jsx(_Fragment, { children: _jsx(RulesetsToggle, { handleOnChange: () => {
                        setAreInjectedRulesetsHidden(!areInjectedRulesetsHidden);
                    }, isActive: !areInjectedRulesetsHidden }, void 0) }, void 0)), Boolean(isFetchingClickedAdvisories || clickedAdvisories.length) && showClickedAdvisoriesBox && (_jsx(SelectedAdvisoriesList, { selectedAdvisories: clickedAdvisories, clearClickedAdvisories: handleClearClickedAdvisories, isFetchingSelectedAdvisories: isFetchingClickedAdvisories, selectedAdvisory: selectedAdvisory, handleSelectAdvisory: handleSelectAdvisory, applyFilterToLayers: applyFilterToLayers, appliedAdvisoriesFilter: appliedAdvisoriesFilter, shouldShowInactiveAdvisories: shouldShowInactiveAdvisories, showInactiveAdvisories: showInactiveAdvisories, selectedAdvisorySettings: selectedAdvisorySettings, selectedAdvisoryHasSmallSize: selectedAdvisoryHasSmallSize }, void 0)), showLocationInformation && (_jsx(LocationInformation, { mapBoundsGeometryCenter: mapBoundsGeometryCenter, locationName: locationName, locationWeather: locationWeather }, void 0))] }, void 0));
};
